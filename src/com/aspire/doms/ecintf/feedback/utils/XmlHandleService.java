 ...
 * 
 */
public class XmlHandleService {

	private static final Logger log = Logger.getLogger("INTERNAL");
	 ...
	 * @throws SAXException
	 */
	private void init(String ctxPath, String xsdName) throws JAXBException,
			SAXException {
		...
		URL url = XmlHandleService.class.getClassLoader().getResource(xsdName);
	 ...
	 * @throws SAXException
	 */
	public XmlHandleService(String ctxPath, String xsdName)
	...
	private void marshal(Object pojo, OutputStream out) throws JAXBException {
		...
		log.info("jaxb marshal a " + pojo.getClass().getSimpleName() + " instance.");
	}
	...
	public String parsePojo(Object pojo, Class<?> clazz) {
		...
		if (!clazz.isInstance(pojo)) {
			log.fatal("Can not parse POJO object as defined type: "
					+ clazz.getName() + ".");
			return null;
		}
		...
		try {
			...
			log.fatal("Can not parse POJO (" + pojo.getClass().getName()
					+ ") to XML string.", e);
			e.printStackTrace();
	...
	public <T> T parseXmlStr(String xmlStr, Class<T> clazz) {
		...
		try {
			...
			if (!clazz.isInstance(tmp)) {
				log.fatal("Can not parse XML string to POJO object as defined type: "
						+ clazz.getName() + ".");
				return null;
			}
			...
			log.fatal(
					"Can not parse XML string to POJO object as defined type use UTF-8 encoding.",
					e);
		} catch (JAXBException e) {
			log.fatal(
					"Can not parse XML string to POJO object as defined type: "
							+ clazz.getName() + ".", e);
		} finally {
			if (stream != null) {
				try {
